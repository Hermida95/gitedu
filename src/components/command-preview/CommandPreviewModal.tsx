  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-lg rounded-lg border border-slate-700 bg-slate-900 p-5 shadow-xl">
        <h2 className="text-base font-bold text-slate-100">{preview.title}</h2>

        <p className="mt-3 text-xs uppercase tracking-wide text-slate-500">Comando que se va a ejecutar</p>
          ...
          <ul className="mt-3 space-y-1 text-sm text-slate-400">
            {preview.impact.map((line) => (
              <li key={line} className="flex gap-2">
                <span className={preview.danger ? 'text-amber-400' : 'text-emerald-400'}>-</span>
                <span>{line}</span>
              </li>
            ))}
          </ul>
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <button
            className={`rounded px-4 py-2 text-sm font-medium disabled:opacity-50 ${
              preview.danger ? 'bg-amber-600 hover:bg-amber-500' : 'bg-emerald-600 hover:bg-emerald-500'
            }`}
            onClick={onConfirm}
            disabled={loading}
          >
