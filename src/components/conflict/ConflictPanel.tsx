...
export function ConflictPanel({
  ...
  return (
    // Sin onClose: a diferencia de los demás modales, aquí no hay "cancelar" — un
    // conflicto real de git solo se resuelve con Continuar o Abortar explícitamente.
    <Modal labelledBy="conflict-panel-title" className="w-full max-w-xl rounded-lg border border-amber-700 bg-slate-900 p-5 shadow-xl">
      <h2 id="conflict-panel-title" className="text-base font-bold text-amber-400">
        Conflicto durante el {label}
      </h2>
          ...
          {conflictState.conflictedFiles.map((file) => (
            <li key={file} className="rounded border border-slate-700 bg-slate-950 p-2">
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  className="rounded border border-slate-700 px-2.5 py-1.5 text-xs hover:bg-slate-800 disabled:opacity-50"
                  disabled={busy}
