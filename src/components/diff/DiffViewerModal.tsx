...
function DiffLine({ line }: { line: string }) {
  ...
  else if (line.startsWith('diff --git') || line.startsWith('index ')) className = 'text-slate-500'
...
export function DiffViewerModal({ repoPath, filePath, status, onClose }: DiffViewerModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="flex max-h-[85vh] w-full max-w-3xl flex-col rounded-lg border border-slate-700 bg-slate-900 p-5 shadow-xl">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="truncate font-mono text-sm font-bold text-slate-100" title={filePath}>
            {filePath}
          </h2>
          <button
            className="shrink-0 rounded border border-slate-700 px-3 py-1 text-xs hover:bg-slate-800"
            onClick={onClose}
          >
            Cerrar
          </button>
        ...
        <div className="flex-1 overflow-auto rounded bg-black py-2 font-mono text-xs">
          ...
          {!error && diff === null && <p className="px-2 text-slate-500">Cargando diff...</p>}
          {!error && diff === '' && <p className="px-2 text-slate-500">Sin diferencias que mostrar.</p>}
